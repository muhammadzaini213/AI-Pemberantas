...
class Vehicle:
    ...
    def _update_state_stats(self, old_state):
        if not self.garage_node or not self.shared:
            return
    ...
    def _is_edge_slow(self, edge_data):
    ...
    def actuator_go_to_tpa(self):
        if not self.TPA_node:
            print(f"[Vehicle {self.id}] No TPA configured")
            return False
        ...
        old_state = self.state
        try:
            path = nx.shortest_path(self.G, self.current, tpa, weight="length")
            ...
            dist = sum(self.G[path[i]][path[i+1]][0]['length'] for i in range(len(path)-1))
            print(f"[Vehicle {self.id}] Route to TPA {tpa} ({dist:.0f}m)")
            return True
        except Exception as e:
            print(f"[Vehicle {self.id}] Failed to route to TPA: {e}")
            return False
    ...
    def actuator_arrive_at_tps(self):
        if self.current in self.TPS_nodes and self.shared:
            ...
            self.state = "at_tps"
            print(f"[Vehicle {self.id}] At TPS {self.current}, found {current_garbage:.2f}kg")
            return True
        return False
    ...
    def actuator_discover_slowdown(self):
        ...
        if hasattr(self.shared, 'edge_type') and edge_id in self.shared.edge_type:
            ...
            if self._is_edge_slow(edge_data):
                slowdown = edge_data.get("slowdown", 0)
            ...
            if idx + 1 < len(self.path):
                ...
                if hasattr(self, '_slowdown_reported'):
                    ...
                    if old_edge and old_edge in self._slowdown_reported:
                        self._slowdown_reported.discard(old_edge)
            else:
                self.current = self.target_node
                self.target_node = None
                self.path = []
