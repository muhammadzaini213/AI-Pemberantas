...
class AIModel:
    ...
    def phase_dispatch(self, vehicles):
        ...
        for vehicle in idle_vehicles:
            ...
            print(f"[AIModel] ASSIGNED {vehicle.id} -> TPS {next_tps} (dist={my_distance:.4f}m, garbage={garbage_info})")
            ...
            if discovered is not None and discovered <= 10:
                ...
                if next_tps:
                    ...
                    if path:
                        ...
                        print(f"[AIModel] REDIRECTED {vehicle.id} -> TPS {next_tps} (dist={dist:.4f}m, garbage={garbage_info})")
        ...
        if remaining_before > 10:
            ...
            if next_tps:
                ...
                if path:
                    ...
                    dist = self._path_distance(path, vehicle.G)
                    print(f"[AIModel] ASSIGNED {vehicle.id} -> TPS {next_tps} (dist={dist:.4f}m, garbage={garbage_info})")
    ...
    def _find_next_tps(self, vehicle):
            ...
            if len(assigned_vehicles) == 0:
                ...
                for assigned_vid in assigned_vehicles:
                    ...
                    if (my_distance < assigned_dist * self.STEAL_DISTANCE_RATIO and 
                        ...
                        print(f"         my_dist={my_distance:.4f}m vs their_dist={assigned_dist:.4f}m (advantage={distance_advantage:.4f}m)")
                        can_take = True
        ...
        if candidates:
            ...
            for i, c in enumerate(candidates[:3]):
                ...
                print(f"  {i+1}. TPS {c['tps_id']}: dist={c['distance']:.4f}m, garbage={c['garbage']:.0f}kg ({status}), score={c['score']:.3f}")
    ...
    def _reassign(self, vehicle):
        ...
        if next_tps:
            ...
            if path:
                ...
                dist = self._path_distance(path, vehicle.G)
                print(f"[AIModel] REASSIGNED {vehicle.id} -> TPS {next_tps} (dist={dist:.3f}m, garbage={garbage_info})")
