...
class TPSStateWindow:
    def __init__(self, master=None):
        ...
        ttk.Checkbutton(frm, text="Dilayanin hari ini", variable=self.served_var, state="readonly").grid(
            row=8, column=0, sticky="w", pady=(0, 12)
        )

        self.save_btn = ttk.Button(frm, text="💾 Save", command=self.on_save)
        self.save_btn.grid(row=9, column=0, sticky="ew")
    ...
    def set_node(self, node_id, data=None):
        ...
        self.sampah_per_hari_var.set(str(data.get("sampah_per_hari", 0)))
        self.sampah_kg_var.set(str(data.get("sampah_kg", 0)))
